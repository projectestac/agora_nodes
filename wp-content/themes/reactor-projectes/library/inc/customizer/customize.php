...
//			 ) );
		...
		// Nommés deixem editar la imatge si el logo
			...
			// Camp per diferenciar els projectes que són Europeus i els que no
			$wp_customize->add_setting('reactor_options[logotipVisible]', array(
				'default'    => false, /*agafar de BBDD agora*/
				'type'       => 'option',
				'capability' => 'manage_options',
				'transport'  => 'postMessage',
			 ) );

			// Camp per seleccionar l'alçada de la barra superior amb el logo
			$wp_customize->add_setting('reactor_options[midaBarra]', array(
				'default'    => '', /*agafar de BBDD agora*/
				'type'       => 'option',
				'capability' => 'manage_options',
				'transport'  => 'postMessage',
			 ) );

			// Camp per fer visible sempre el logo i indicar l'alçada de la barra. Només visible per a l'xtecadmin
			if(is_xtecadmin()) {
				$wp_customize->add_control('reactor_options[logotipVisible]', array(
					'label'    => __('Logotip fixe', 'reactor'),
					'section'  => 'reactor_customizer_idcentre',
					'priority' => 2,
					'type'	   => 'checkbox',
				 ) );

				$wp_customize->add_control('reactor_options[midaBarra]', array(
					'label'    => __('Alçada del logotip fixe', 'reactor'),
					'section'  => 'reactor_customizer_idcentre',
					'priority' => 2,
				 ) );
			}
			...
			$wp_customize->add_section('reactor_customizer_colors', array(
