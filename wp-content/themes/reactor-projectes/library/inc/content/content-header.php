...
function reactor_do_title_logo()
{
    ...
    // XTEC ************ AFEGIT - Check and apply modifications to europeans project
    ...
    if( isset($projectes) && $projectes['logotipVisible'] === true ){
        // Get name blog
        $blog_info = nl2br(get_option('nodesbox_name'));
        // Get description blog
        $blog_description = get_bloginfo('description');
        if( strlen($blog_description) > 50 ){
            $blog_description = substr($blog_description,0,50).'...';
        }
    ?>
        <div id="scroll-on" class="row xtec_float_bar">
            <div class="xtec-content-header xtec-content-logo">
                <img src="<?php echo $projectes['logo_image']; ?>" class="project_logo">
            </div>
            <div class="xtec-content-header xtec-content-text">
                <p class="xtec-text-logo"><?php echo $blog_info; ?>&nbsp;&nbsp;&nbsp;<span class="xtec_blog_description"><?php echo $blog_description ?></span></p>
            </div>
        </div>
    <?php
    }
