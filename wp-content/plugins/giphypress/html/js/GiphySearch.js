...
var GiphySearch = {
    ...
    init: function(data) {
        ...
        // Google Analytics
        var config = {
            account : 'UA-38174542-28',
            loadGA  : true,
            name    : 'giphy-press',
            api_key: GiphySearch.API_KEY
        };

        gat = new GAT(config);

        GiphySearch.bind_events();
        GiphySearch.bindTrackingEvents();
    ...
    bind_events:function() {
        ...
        }).on("click", "#categories .subtitle", function(event) {
            GiphySearch.expandCategory(event);
        }).on("click", "#categories .popular_tag", function(event) {
            GiphySearch.handleTag.call(this,event);
        });

        // Watch hover for tracking purposes
        $container.on("mouseover", ".giphy_gif_li .gif_drag_cover", GiphySearch.handleGIFHover);

        jQuery('.embed-button').on("click", function(event){
            var gif_id = jQuery('img#gif-detail-gif').attr('data-id');
            gat.track('Send', 'Click', gif_id);
            gat.log_action("SENT", gif_id, Date.now());
            //Close the session once a GIF has been sent.
            gat.send_pingback();
            // This is not intuitive -- but since we can't track modal close, create a new event with the same search response ID
            gat.create_new_event(mostRecentRespId);
            setTimeout(function(){
    ...
    bindTrackingEvents:function(){
        jQuery('.popular_tag').on("click", function(){
            gat.track('Search', 'Popular Tag Click', jQuery(this).data('tag'));
        })

        jQuery('#categories-button').on("click", function(){
            gat.track('Search', 'Categories', 'Menu Click');
        })

        jQuery('.subtitle').on("click", function(){
            var subtitle = jQuery(this).attr('id');
            gat.track('Search', 'Categories', subtitle);  
        });
    },
    ...
    init_chrome_extension:function() {
        ...
        jQuery("#container").on("click", "#gif-detail-link", function(event) {
            ...
            // google analytics
            // gat.track('Click', 'copy:link', $this.data("bitly_gif_url"))
    ...
    expandCategory: function(event) {
        var className = jQuery(event.target).attr('id').replace('subtitle-', '');
        if (!jQuery(event.target).attr('data-open') || jQuery(event.target).attr('data-open') == 'false') {
            jQuery(event.target).attr('data-open', 'true');
            jQuery('.'+className+'-tag').show();
        } else {
            jQuery(event.target).attr('data-open', 'false');
            jQuery('.'+className+'-tag').hide();
        }
    },
    ...
    */
    ...
    handleTag: function(event) {
        ...
        if(tag == '') return;
        if (!jQuery(event.target).hasClass('popular_tag')){
            gat.track('Search', 'Tag Click', tag);  
        }
        GiphySearch.show_preloader();
    ...
    handleGIFHover: function(event){
        gat.log_action("HOVER", jQuery(event.target).parent(".giphy_gif_li").find("img").attr("data-id"), Date.now());
    },
    ...
    _opendetail:function(gif) {
        ...
        gat.log_action("CLICK", gif.attr("data-id"), Date.now());
        GiphySearch.show_preloader();
            ...
            (function($li, _pos) {
                ...
                // if((liTop >= windowTop - liHeightOffset) && (liBottom <= windowBottom + liHeightOffset)) {
                    ...
                    if (!jQuery(img).hasClass("seen")){
                        ...
                        gat.log_action("SEEN", jQuery(img).attr("data-id"), Date.now());
                    }
    ...
    process_search_response:function(response) {
        ...
        gat.create_new_event(response.meta.response_id);
