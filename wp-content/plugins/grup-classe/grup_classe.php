...
/*
 ...
 Description: Afegeix un nou giny que inclou elements útils per un blog d'un grup-classe 
 (calendari, informació sobre tutoria, formulari de subscripció, enllaços associats) i dos blocs de text/HTML
 Version: 1.1
 Author: Xavier Meler
...
class grup_classe_widget extends WP_Widget {
    ...
    function form($instance) {
        ...
        $ig_forms        = isset($instance['ig_forms']) ? $instance['ig_forms'] : '';
        ...
        <div>
            ...
            <input id="<?php echo $this->get_field_id('nom_calendari'); ?>" name="<?php echo $this->get_field_name('nom_calendari'); ?>" type="text" value="<?php echo esc_attr($nom_calendari); ?>">
            <br />
            <?php _e("Calendar:", "grup-classe"); ?><br />
            <select id="<?php echo $this->get_field_id('id_calendari'); ?>" name="<?php echo $this->get_field_name('id_calendari'); ?>">
        ...
        <div>
            ...
            <input name="<?php echo $this->get_field_name('nom_tutor'); ?>" type="text" value="<?php echo esc_attr($nom_tutor); ?>" /><br />
            <?php _e("Tutor's email:", "grup-classe"); ?><br />
            <input name="<?php echo $this->get_field_name('email_tutor'); ?>" type="text" value="<?php echo esc_attr($email_tutor); ?>" /><br />
            <?php _e("Timetable for attending to families:", 'grup-classe'); ?><br />
            <input name="<?php echo $this->get_field_name('horari_families'); ?>" type="text" value="<?php echo esc_attr($horari_families); ?>" /><br />
        </div>
        <br />
        ...
        <div>
            ...
            <select id="<?php echo $this->get_field_id('ig_forms'); ?>" name="<?php echo $this->get_field_name('ig_forms'); ?>">
                <option value="0"></option>
                <?php
                        foreach ($subscription_groups as $subscription_group) {
                            echo '<option value="' . $subscription_group->id . '"'
                                . selected($ig_forms, $subscription_group->id, false)
                                . '>' . esc_html($subscription_group->name) . '</option>';
                        }
                        ?>
            </select>
        </div>
...
<?php
    ...
    function widget($args, $instance) {
        ...
        $ig_forms        = (empty($instance['ig_forms'])) ? '' : $instance['ig_forms'];
        ...
        if (!empty($nom_tutor) || !empty($email_tutor) || !empty($horari_families)) {
            ...
            if (!empty($horari_families))
                echo "<li><span class='dashicons dashicons-clock'></span>$horari_families</li>";
            echo '</ul>';
        }
