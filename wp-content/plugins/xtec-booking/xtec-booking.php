...
// Delete post meta for resources and bookings when post is deleted.
add_action('wp_trash_post', function ($postid) {
    $data = get_post_meta($postid);

    if (isset($data['_xtec-booking-resource'], $data['_xtec-booking-data'])) {
        delete_post_meta($postid, '_xtec-booking-resource');
        delete_post_meta($postid, '_xtec-booking-data');
...
add_action('admin_head', function (): void {
    ...
    if (!empty($post) || $check_calendar_booking !== false || $check_xtec_booking !== false) {
        ...
        if ((isset($post->post_type) && $post->post_type === 'calendar_booking') || $post->post_type === 'calendar_resources' ||
