...
function xtec_custom_columns( $name ) {
    ...
    switch ( $name ){
        case 'resource':
            ...
            if( $status == 'inactive' ){
            	echo $views->post_title.'&nbsp;&nbsp;&nbsp;&nbsp;<br><small style="color:red;font-size:10px"><strong><i>'.__('Not available resource','xtec-booking').'</i></strong></small>';
            } else if ( $wpStatus == 'private'){
            	$roles = wp_get_current_user();
				if ( ! in_array( 'administrator', (array) $roles->roles ) ){
					echo $views->post_title.'&nbsp;&nbsp;&nbsp;&nbsp;<br><small style="color:red;font-size:10px"><strong><i>'.__('Only admin users','xtec-booking').'</i></strong></small>';
				} else {
