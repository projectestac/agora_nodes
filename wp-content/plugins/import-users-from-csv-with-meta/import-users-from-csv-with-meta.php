...
//************ FI
        while ($data = $manager->fgetcsv($delimiter)):
            ...
            //2015.03.30 @nacho
                    -->
                    <tr><th><?php _e('Row', 'import-users-from-csv-with-meta'); ?></th><?php foreach ($headers as $element)
...
function acui_options() {
    if (!current_user_can('edit_users')) {
        ...
        //XTEC ************ MODIFICAT - Added language supporting
        //2015.03.20 @nacho
                    alert("<?php $msg = _e("Please choose a file", "import-users-from-csv-with-meta");
        ;
        echo $msg; ?>");
        //************ ORIGINAL
                ...
                while (list($line_num, $line) = each($tmp)) {
                    if (preg_match("/" . $escape . $enclosure . "/", $line)) {
                        while ($strlen = strlen($line)) {
                            ...
                            if (
                                is_int($pos_delimiter) && is_int($pos_enclosure_start) && ($pos_enclosure_start < $pos_delimiter)
                            ) {
                                ...
                                if (empty($pos_delimiter) && empty($pos_enclosure_start)) {
                                    ...
                                    $offset = (
                                        !empty($pos_enclosure_start) && ($pos_enclosure_start < $pos_delimiter)
