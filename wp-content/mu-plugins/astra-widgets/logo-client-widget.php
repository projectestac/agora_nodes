...
class Logo_Client_Widget extends WP_Widget {

    // Create widget
    public function __construct() {
        parent::__construct(
            'logo_client_widget', // ID base
            __('Client card', 'astra-nodes'), // Name
            ['description' => 'Mostra les dades i, opcionalment, el logotip del centre. La informació es defineix a Aparença -> Personalitza -> Identificació del centre']
        );
    }
    ...
    public function widget($args, $instance): void {
        ...
        if (!$contacte && $contacte_mobile) {
            $contacte = $contacte_mobile;
            $contacte_mobile_enabled = true;
        } else if (!$contacte_mobile && $contacte) {
        ...
        <div class="targeta_id_centre row">
            ...
            if (reactor_option('logo_image')) {
                ...
                ?>
                <div class="<?php echo $class_logo; ?> hide-for-small">
                    <img alt="Logotip entitat" id="logo_entity" src="<?php
            ...
            <?php
            [$postal_code, $locality] = explode(' ', reactor_option('cpCentre'), 2);
            ?>
            <div class="<?php echo $class_addr; ?> ">
                <div class="vcard">
                    <span id="tar-nomCentre"><?php
                        echo reactor_option('nomCanonicCentre'); ?></span>
                    <div class="adr">
                        <span class="street-address"><?php
                            echo reactor_option('direccioCentre'); ?></span><br>
                        <span class="postal-code"><?php
                            echo trim($postal_code); ?></span>
                        <span class="locality"><?php
                            echo trim($locality); ?></span>
                        <span class="region" title="Catalunya">Catalunya</span>
                        <span class="country-name">Espanya</span>
                        <div class="tel">
                            <span><?php
                                echo reactor_option('telCentre'); ?></span>
                        </div>
                        <?php

                        // Get home url to check behavior target link
                        $currentDomain = get_home_url();
                        $searchDomain = ['http://', 'https://'];
                        ...
                        if (!empty($emptyMaps)) {
                            ...
                            if (strpos(reactor_option('googleMaps'), $currentDomain) !== false) {
                                ...
                                echo reactor_option('googleMaps'); ?>">mapa</a>
                                <?php
                            } else if (strpos(reactor_option('googleMaps'), 'http') === false) {
                                if (strpos(reactor_option('googleMaps'), '.') !== false) {
                                    ?>
                                    <a id="tar-mapa" target="_blank" href="<?php
                                    echo esc_url("http://" . reactor_option('googleMaps')); ?>">mapa</a>
                                    <?php
                                } else if (strpos(trim(reactor_option('googleMaps')), '/') === 0) {
                                    ?>
                                    <a id="tar-mapa" href=" <?php
                                    echo esc_url(get_home_url() . reactor_option('googleMaps')); ?>">mapa</a>
                                    <?php
                                } else {
                                    ?>
                                    <a id="tar-mapa" href=" <?php
                        ...
                        if ($contacte && $contacte_mobile) {
                            ...
                            if ($contacte_mobile_enabled) {
                                ?>
                                <a id="tar-contacte" href="<?php
                                echo esc_url($contacte); ?>">contacte</a>
                                <?php
                            } else if (strpos($contacteDomain, $currentDomain) !== false) {
                                ?>
                                <a id="tar-contacte" href="<?php
                                echo $contacte; ?>">contacte</a>
                                <?php
                            } else {
                                if (strpos($contacte, 'http') === false) {
                                    if (strpos($contacte, '.') !== false) {
                                        ?>
                                        <a id="tar-contacte" target="_blank" href="<?php
                                        echo esc_url("https://" . $contacte); ?>">contacte</a>
                                        <?php
                                    } else if (strpos(trim($contacte), '/') === 0) {
                                        ?>
                                        <a id="tar-contacte" href="<?php
                                        echo esc_url($currentDomain . $contacte); ?>">contacte</a>
                                        <?php
                                    } else {
                                        ?>
                                        <a id="tar-contacte" href="<?php
...
add_action('widgets_init', function () {
    register_widget('logo_client_widget');
}
);
