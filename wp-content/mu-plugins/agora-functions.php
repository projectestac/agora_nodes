...
function default_theme_mod(): array {
    ...
    $organism_logo = stripos($reactor_options['cpCentre'], 'barcelona') ? 'ceb' : 'department';
...
function migrate_logo($image_url): int {
    ...
    if (is_array($astra_settings)) {
        $astra_settings['custom_logo'] = $reactor_logo_id;
...
function initialize_palettes(): void {
    ...
    if ($astra_color_palettes !== false && !empty($palettes)) {
...
if (get_option('notify_admin_on_pending_submission', 1)) {
